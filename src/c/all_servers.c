// src/c/all_servers.c
// Full server list with folder support. Identical to the v1.1 server_list
// behavior; it now lives behind the "Show all servers" entry on the home page.
...
// Row buffers live on the heap so they don't take a chunk of the 64KB image
// size budget (uint16 virtual_size cap — see HANDOFF gotcha #3).
static SRow *s_all;
...
static void rebuild_visible(void) {
  if (!s_visible || !s_all) return;
  s_visible_count = 0;
  for (int i = 0; i < s_all_count; i++) {
    SRow *r = &s_all[i];
    bool show = (r->parent < 0) || wc_csv_contains(s_expanded, s_all[r->parent].id);
...
static void on_rows_done(WcRow *rows, int count) {
  ...
  for (int i = 0; i < count && s_all_count < WC_MAX_ROWS; i++) {
    ...
    if (r->parent >= s_all_count) r->parent = -1;
    r->n_members = 0;
    if (w->n_fields >= 6 && w->fields[5][0]) {
      const char *p = w->fields[5];
      while (*p && r->n_members < 4) {
        r->members[r->n_members++] = wc_hex_to_color(p);
        while (*p && *p != ',') p++;
        if (*p == ',') p++; else break;
      }
    }
...
static void draw_status(GContext *ctx, GRect b) {
  ...
  if (s_state == ST_EMPTY) msg = "No servers.";
...
static void draw_row(GContext *ctx, const Layer *cell_layer, MenuIndex *ci, void *ctx2) {
  ...
  if (r->kind == 'f') {
    ...
    GPoint disc_c = GPoint(b.origin.x + 6 + indent + 11, b.origin.y + b.size.h / 2);
...
static void window_load(Window *w) {
  s_all = (SRow*)malloc(sizeof(SRow) * WC_MAX_ROWS);
  s_visible = (int*)malloc(sizeof(int) * WC_MAX_ROWS);
  if (!s_all || !s_visible) { s_state = ST_ERROR; s_err_code = 3; }
